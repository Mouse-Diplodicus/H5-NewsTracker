...
class TestMain(unittest.TestCase):
    ...
    @classmethod
    def setUp(cls):
        cls.library = [["headline_0", "https://test_0.com"], ["headline_1", "https://test_1.com"],
                        ["headline_2", "https://test_2.com"]]
        cls.test_config = utilities.load_config_file()['ticker_window']

    @patch('H5_News_Tracker.controller.main.Controller')
    ...
    def test_url_list_from_file(self):
        ...
        """
        test_result = main.url_list_from_file('urls.txt')
        if test_result is None:
            test_result = main.url_list_from_file('tests/urls.txt')
    ...
    def test_build_library(self):
        with patch('H5_News_Tracker.parser.feed_interface.build_library') as mocked_build_library:
            with patch('H5_News_Tracker.parser.feed_interface.parse') as mocked_parser:
                main.build_library(['https://news.google.com/news/rss'])
                mocked_build_library.assert_called_with(mocked_parser('https://news.google.com/news/rss'))

    def test_parse_args(self):
        pass

    @patch("H5_News_Tracker.controller.main.Controller.cycle")
    def test_controller_init(self, mock_cycle):
        ...
        with patch('H5_News_Tracker.gui.ticker_window.TickerWindow') as mocked_ticker_window:
            main.Controller(self.library, cycle_time=3, ticker_config=self.test_config)
            self.assertTrue(mock_cycle.called)
            mocked_ticker_window.assert_has_calls(mocked_ticker_window.start())

    @patch("H5_News_Tracker.controller.main.TickerWindow.update_headline")
    def test_cycle(self, mock_update_headline):
        ...
        test_lib = [["headline_0", "https://test_0.com"], ["headline_1", "https://test_1.com"],
                        ["headline_2", "https://test_2.com"]]
        controller = main.Controller(test_lib, cycle_time=2, ticker_config=self.test_config)
    ...
    def test_start_gui(self):
        with patch('H5_News_Tracker.gui.ticker_window.TickerWindow') as mocked_ticker_window:
            control = main.Controller(self.library, cycle_time=3, ticker_config=self.test_config)
