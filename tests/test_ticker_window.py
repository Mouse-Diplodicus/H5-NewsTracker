...
class TestTickerWindow(unittest.TestCase):
    ...
    def test_start(self):
        with patch('H5_News_Tracker.gui.ticker_window.TickerWindow') as self.mocked_ticker:
            self.mocked_ticker()
            self.mocked_ticker.start()
            # self.mocked_ticker.start.assert_has_calls(root.update())
